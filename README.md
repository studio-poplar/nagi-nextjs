...
- **パスワード変更**：ログイン中の画面右上「パスワード変更」から、現在のパスワードを入力して新しいパスワードに変更可能
...
管理画面内の「パスワード変更」機能を一度も使っていなければ、Vercelの`ADMIN_PASSWORD`環境変数だけがログインを制御しています。

1. Vercel の Settings → Environment Variables で `ADMIN_PASSWORD` の値を新しいものに上書き（現在の値が分からなくても上書き保存できます）
2. Redeploy（または次のpushで自動反映）
3. 新しいパスワードでログイン

管理画面内の「パスワード変更」を一度でも使っていた場合は、そのパスワードのハッシュが `content/auth.json` としてリポジトリにコミットされ、`ADMIN_PASSWORD` より優先されます。その場合は上記に加えて、GitHub上で `content/auth.json` を削除（Web UIから該当ファイルを開いて Delete）してから同様に再デプロイしてください。
